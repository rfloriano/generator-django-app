...
var PythonPackageGenerator = yeoman.generators.Base.extend({
  ...
  askFor: function (mute, callback) {
    ...
    getUserNameAndEmail(function() {
      var prompts = [{
        type: 'input',
        name: 'packageName',
        message: 'Django app name (the name that will be used when sending to pypi):',
        default: currentPath
      }, {
        type: 'input',
        name: 'authorName',
        ...
        default: 'an incredible django app',
        filter: escapeQuotes
      }, {
        type: 'input',
        name: 'keywords',
  ...
  app: function (currentPkg) {
    ...
    // pkg.name/pkg.name
    this.mkdir(pkg.pythonName + '/' + pkg.pythonName);
    this.template('_django_app/_django_app/_init.py', pkg.pythonName + '/' + pkg.pythonName + '/__init__.py');
    this.template('_django_app/_django_app/_settings.py', pkg.pythonName + '/' + pkg.pythonName + '/settings.py');
    this.template('_django_app/_django_app/_urls.py', pkg.pythonName + '/' + pkg.pythonName + '/urls.py');
    this.template('_django_app/_django_app/_version.py', pkg.pythonName + '/' + pkg.pythonName + '/version.py');
    this.template('_django_app/_django_app/_wsgi.py', pkg.pythonName + '/' + pkg.pythonName + '/wsgi.py');

    // tests/
    this.mkdir('tests/');
    this.template('_tests/_init.py', 'tests/__init__.py');
    this.template('_tests/_base.py', 'tests/base.py');
    this.template('_tests/_test_version.py', 'tests/test_version.py');
  },
